...
void print_mins(int* mins, int n)
{
...
void print_answer_file(int** Matrix, int n, int* mins)
{
	const int m = 10;
	printf("\n Введите название файла: ");
	char name[11];
...
void delete_doubles(int** Matrix, int* n, int* mins)
{
	int m = 10;
	for (int i = 0; i < *n; i++) // основной шаг по строкам
		for (int j = 0; j < m; j++) // береём один элемент
			for (int k = 0; k < m; k++) // и сравниваем его со всеми остальными
				if (Matrix[i][j] == Matrix[i][k] && j != k) { // если элементы строки равны между собой и это не один и тот же элемент
					for (int f = i; f < *n - 1; f++) { // линйеный сдвиг (удаление строки)
						Matrix[f] = Matrix[f + 1];
						mins[f] = mins[f + 1];
					}
					(*n)--; // уменьшаем общее кол-во строк на один
					i--; // уменьшаем текущий индекс
					break; // возвращаемся к
					break; // шагу по строкам
				}
}
...
void task3()
{
	...
	do {
		...
		case 2:
			...
			int *mins = new int[n]; // массив минимальных элементов
			...
			sort_strings(Matrix, mins);
			delete_doubles(Matrix, &n, mins);
			...
			{
			case 1:
				print_answer_file(Matrix, n, mins);
				break;
			case 2:
